...
#fazer smote 0 e 1 classes e outro com classes 0 e 2
subset <- train_data[train_data$Result %in% c(0,1), ]    #dataset so com class = 0 e 1 (238  56)
over_1 <- SMOTE(subset[, -30], subset$Result, K = 5)
over_1 <- over_1$data  # 0  1  =  238  224
subset2 <- train_data[train_data$Result %in% c(0,2), ]   #dataset so com class = 0 e 2 (238  10)
over_2 <- SMOTE(subset2[, -30], subset2$Result, K = 5)
over_2 <- over_2$data  # 0  1  =  238  230


library(dplyr)
merged_data <- full_join(over_1, over_2)
colnames(merged_data)[30] <- "Result"
table(merged_data$Result)       # 0  1  2 = 238  224  230
#total=692 obs


# Normalizar os dados
library(caret)
preprocessed_smote <- preProcess(merged_data[,c(3,5,7:17)], method = c("range"))
scaledTrainData <- predict(preprocessed_smote, newdata = merged_data)
...
write_xlsx(scaledTrainData, "C:/Users/ASUS/OneDrive/Train_categorical.xlsx")
write_xlsx(scaledTestData, "C:/Users/ASUS/OneDrive/Test_categorical.xlsx")


####### RLM SEM FEATURE SELECTION USING SMOTE ###########
...
#PROBLEMA: P-Value [Acc > NIR] : 1.00000   nao significativo, pode ser devido ao facto de utilizar todas as variaveis independentes?
#deve levar a overfitting -> + complexidade 
#dai devemos aplicar feature selection ou metodos de regulariza?ao
